...
function useSlot(name, parameters) {
  ...
  _objectWithoutPropertiesLoose(externalForwardedProps, _excluded2$1);
...
const AvatarFallback = styled$1(Person$1, {
  name: "MuiAvatar",
  slot: "Fallback",
...
const useAuth = () => {
  ...
  const changeAccessJwt = ({ accessJwt: accessJwt2 }) => {
...
const useProfile = () => {
  ...
  const changeAvatar = ({ avatar: avatar2 }) => {
    setAvatar(avatar2);
  };
  const [displayName, setDisplayName] = React.useState("");
  const changeDisplayName = ({ displayName: displayName2 }) => {
    setDisplayName(displayName2);
  };
  const [handle, setHandle] = React.useState("");
  const changeHandle = ({ handle: handle2 }) => {
    setHandle(handle2);
  };
  const [description, setDescription] = React.useState("");
  const changeDescription = ({ description: description2 }) => {
    setDescription(description2);
  };
  const [followersCount, setFollowersCount] = React.useState(0);
  const changeFollowersCount = ({ followersCount: followersCount2 }) => {
    setFollowersCount(followersCount2);
  };
  const [followsCount, setFollowsCount] = React.useState(0);
  const changeFollowsCount = ({ followsCount: followsCount2 }) => {
    setFollowsCount(followsCount2);
  };
...
const useFeed = () => {
  ...
  const changeTimeLine = ({ timeLine: timeLine2 }) => {
...
if (document.getElementById("index")) {
  ...
  root.render(/* @__PURE__ */ jsxRuntimeExports.jsx(Index, {}));
}
